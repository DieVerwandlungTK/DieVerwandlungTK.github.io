 ...
 * the next step. Both halves use the same force -- the one evaluated at the configuration this
 * step ends on -- so together they are a *full* dt kick: the kick below carries `dt`, not
 * `0.5 * dt`, even though each drift carries `0.5 * dt`. Halving it makes every molecule feel
 * half the force while the thermostat still injects noise for the full set point, which samples
 * the potential at twice the set temperature.
 */
@compute @workgroup_size(64)
